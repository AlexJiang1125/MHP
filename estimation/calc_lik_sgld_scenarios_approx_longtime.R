...
filename <- paste0("/gscratch/csde/jiang14/output_sensitivity/scenario_",id_s, "/mhpvisgld_appr_longtime_seed_",id_i,id_k,id_l,id_s,".csv")
if (file.exists(filename)) {
  ...
  for (id_l in id_l: id_l) {
    ...
    if (type == "local") {
      load(paste0(foldernames[id_l],"/mhpvisgld_appr_longtime_seed_",seeds[id_i],"_dataset_",datasets[id_j],"_time",times[id_k],".RData"))
    } else {
      if (id_j <= 20) {
      ...
      load(paste0("/gscratch/csde/jiang14/output_sensitivity/scenario_",id_s,"/",foldernames[id_l],"/mhpvisgld_appr_longtime_seed_",id_i,"_dataset_",id_j,"_time",times[id_k],".RData"))
    }
...
if (type == "local") {
  ...
  write.csv(df_out, file = paste0("/gscratch/csde/jiang14/output_sensitivity/scenario_",id_s, "/mhpvisgld_appr_longtime_seed_",id_i,id_k,id_l,id_s,".csv"))
} 
