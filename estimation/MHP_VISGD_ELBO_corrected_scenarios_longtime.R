...
ids<-expand.grid(seed = 1:16, dataset = 1:50, scenarios = c(1,3))

taskid <-ids$seed[taskid_org]
dataset_seed <- ids$dataset[taskid_org]
scenarioid <- ids$scenarios[taskid_org]
print(taskid)
print(dataset_seed)

for (split_id in 11:11) { 
  for (dataset_seed in dataset_seed: dataset_seed) {
    if (type == "cluster") {
      ...
      load(
        paste0(
          "/Users/alexziyujiang/Documents/data/data_smaller/data_",
          dataset_seed,
          ".RData"
        )
      )
    ...
    print(paste0("learning parameters: ", sgdtr_par1, ",", sgdtr_par2))
    out_all <- out
    ...
    filename <- paste0(
          "/gscratch/csde/jiang14/output_sensitivity/scenario_", scenarioid, "/s",
          files[split_id],
          "/mhpvisgdtr_seed_",
          taskid,
          "_dataset_" ,
  ...
  while (time < 30) {
    ...
    if (count <= offset) {
      ...
      rho <-
        1 * (count - offset - 1 + sgdtr_par1) ^ (-sgdtr_par2) # step size parameter
    }
    ## scan through batch
