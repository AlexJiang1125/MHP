...
if (type == "local") {
  ...
  #source("/gscratch/csde/jiang14/R/sim001/MHP.R")
...
for (dataset_seed in 1: 1) {
  if (type == "cluster") {
    if (mode == "medium") {
      ...
      load(
        paste0(
          "/gscratch/csde/jiang14/data/sim001/data/data_app/",
          "data_app" ,
          ".RData"
        )
      )
    ...
    load("/Users/alexziyujiang/Dropbox/data/mhp_app/reddit/data_app.RData")
  }
  #load(paste0("/gscratch/csde/jiang14/data/sim001/data/data_",dataset_seed,".RData"))
  print(taskid)
  set.seed(taskid)
  time_EMLB <- 0
  sgd_par1 <- 1
  ...
  K <- 11
  # alpha_inits <- matrix(rgamma(K*K, e_alpha, f_alpha), K ,K)
  # beta_inits <- matrix(rgamma(K*K, r_beta, s_beta), K ,K)
  # mu_inits <- rep(rgamma(K, a_mu, b_mu), K)
  # 
  
  if (type == "cluster") {
    load(
      paste0(
        "/gscratch/home/jiang14/mhp/data/sim001/EM_app_inits/inits_",
        taskid,
        ".RData"
      )
    )
    ...
    load(
      paste0(
        "/Users/alexziyujiang/Dropbox/data/mhp_app/EM_app_inits/inits_",
        taskid,
        ".RData"
      )
    )
  ...
  s_mu1 <- rep(0,K)
  s_mu2 <- rep(0,K)
  s_alpha1 <- matrix(0,K,K)
  s_alpha2 <- matrix(0,K,K)
  s_beta1 <- matrix(0,K,K)
  s_beta2 <- matrix(0,K,K)
  Nall <- length(out_all$dim)
  pos <- 1
  count <- 1
  offset <- 1
  maxiters <- 1000
  ...
  while (time < 10) {
    ...
    for (sb in 1:1) {
      ...
      out_truncated <- out[out$timestamp <= T*split_ratio - 0, ]
      out_tail <- out[out$timestamp > T*split_ratio - 0, ]
