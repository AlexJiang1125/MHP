...
slice <- function(x,pos) {
  K <- 11
  return(matrix(unlist(x), K, K, byrow = TRUE))
}
...
for (dataset_seed in 1: 1) {
  if (type == "cluster") {
    ...
    load(
      paste0(
        "/Users/alexziyujiang/Documents/data/data_app/",
        "data_app",
        ".RData"
      )
    )
  ...
  if (type == "cluster") {
    load(
      paste0(
        "/gscratch/home/jiang14/mhp/data/sim001/VI_app_inits/inits_",
        taskid,
        ".RData"
      )
    )
    ...
    load(
      paste0(
        "/Users/alexziyujiang/Documents/data/VI_app_inits/inits_",
        taskid,
        ".RData"
      )
    )
