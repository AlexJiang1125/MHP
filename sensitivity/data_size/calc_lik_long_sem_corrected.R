...
for (id_j in 1:50) {
  ...
  for (id_l in id_l:id_l) {
    ...
    if (type == "local") {
      ...
      load(paste0("/gscratch/csde/jiang14/output_long/", foldernames[id_l],"/mhpvisemtr_seed_",id_i,"_dataset_",id_j,"_time",times[id_k],".RData"))
    }
...
if (type == "local") {
  ...
  write.csv(df_out, file = paste0("/gscratch/csde/jiang14/output_long/mhpvisemtr_seed_",id_i,id_k,id_l,".csv"))
}
}
