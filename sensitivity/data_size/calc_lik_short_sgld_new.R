...
for (id_k in id_k: id_k) {
  for (id_j in 1:50) {
    if (type == "local") {
      load(paste0("/Users/alexziyujiang/Documents/data/data/data_",id_j,".RData"))
    } else {
      load(
        paste0(
   ...
   filename <- paste0("/gscratch/csde/jiang14/output_short/mhpvisgld_seed_",id_i,id_k,id_l,".csv")
   ...
   #break
    ...
    for (id_l in id_l:id_l) {
      ...
      if (type == "local") {
        load(paste0(foldernames[id_l],"/mhpvisgld_seed_",id_j,"_dataset_",id_i,"_time",times[id_k],".RData"))
      } else {
        load(paste0("/gscratch/csde/jiang14/output_short/", foldernames[id_l],"/mhpvisgld_seed_",id_i,"_dataset_",id_j,"_time",times[id_k],".RData"))
      }
      ...
      mu_arrays <- apply(mu_arrays, MARGIN =  c(1), FUN = "median")
...
if (type == "local") {
  write.csv(df_out, file = paste0("mhpvisgld_seed_",id_i,".csv"))
} else if (type == "cluster") {
  write.csv(df_out, file = paste0("/gscratch/csde/jiang14/output_short/mhpvisgld_seed_",id_i,id_k,id_l,".csv"))
}
