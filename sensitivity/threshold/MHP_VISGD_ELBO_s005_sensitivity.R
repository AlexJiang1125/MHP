...
pars <- expand.grid(seed_id = 1:16, dat_id = 1:50, thres_id = 1:4)
seed_id <- pars$seed_id[taskid]
dat_id <- pars$dat_id[taskid]
thres_id <- pars$thres_id[taskid]

slice <- function(x,pos) {
  return(matrix(c(x[[1]][[1]][1], x[[1]][[2]][1], x[[1]][[3]][1],
                  x[[2]][[1]][1], x[[2]][[2]][1], x[[2]][[3]][1],
                  x[[3]][[1]][1], x[[3]][[2]][1], x[[3]][[3]][1]), nrow = 3, byrow = TRUE))
}
ELBO <-
  function(eta_mu1,
           eta_mu2,
    ...
    for (i in 1:K) {
      for (j in 1:K) {
        ...
        res <-
          res + (digamma(eta_alphas1[[i]][[j]][1]) - log(eta_alphas2[[i]][[j]][1])) *
          (a_mu - eta_alphas1[[i]][[j]][1] + ntrans)
        res <-
          res + (digamma(eta_betas1[[i]][[j]][1]) - log(eta_betas2[[i]][[j]][1])) *
          (e_alpha - eta_betas1[[i]][[j]][1] + ntrans)
        res <-
          res - exp(log(eta_alphas1[[i]][[j]][1]) - log(eta_alphas2[[i]][[j]][1])) *
          (sum(out_dat$dim == i) + f_alpha - eta_alphas2[[i]][[j]][1])
        res <-
          res + lgamma(eta_alphas1[[i]][[j]][1]) - eta_alphas1[[i]][[j]][1] *
          log(eta_alphas2[[i]][[j]][1])
        res <-
          res + lgamma(eta_betas1[[i]][[j]][1]) - eta_betas1[[i]][[j]][1] * log(eta_betas2[[i]][[j]][1])
        dists <- 0
        for (ki in 2:N) {
          for (kj in 1:(ki - 1)) {
        ...
        res <-
          res - exp(log(eta_betas1[[i]][[j]][1]) - log(eta_betas2[[i]][[j]][1])) *
          (dists + s_beta - eta_betas2[[i]][[j]][1])
      }
    }
    for (i in 1:K) {
      res <-
        res + (digamma(eta_mu1[1, i]) - log(eta_mu1[1, i])) * (sum(diag(B_dat) *
                                                                     (out_dat$dim == i)) + a_mu - eta_mu1[1, i])
      res <-
        res + lgamma(eta_mu1[1, i]) - eta_mu1[1, i] * log(eta_mu2[1, i])
      res <-
        res - eta_mu1[1, i] / eta_mu2[1, i] * (T / 10 + b_mu - eta_mu2[1, i])
    }
...
for (dat_id in dat_id: dat_id) {
  ...
  while (time < 30) {
    ...
    for (sb in 1:1) {
      ...
      B <- matrix(0, N, N)
      B[1, 1] <- 1
      sliced_eta_alphas1 <- slice(eta_alphas1,1) #pos-1
      ...
      for (ii in 2:N) {
        ...
        # for (jj in 1:(ii - 1)) {
        #   ptemp[jj] <-
        #     digamma(eta_alphas1[[out$dim[jj]]][[out$dim[ii]]][pos - 1]) -
        #     log(eta_alphas2[[out$dim[jj]]][[out$dim[ii]]][pos - 1]) +
        #     digamma(eta_betas1[[out$dim[jj]]][[out$dim[ii]]][pos - 1]) -
        #     log(eta_betas2[[out$dim[jj]]][[out$dim[ii]]][pos - 1]) -
        #     eta_betas1[[out$dim[jj]]][[out$dim[ii]]][pos - 1] / eta_betas2[[out$dim[jj]]][[out$dim[ii]]][pos - 1] * (y[ii] - y[jj])
        # }
  ...
  if (type == "cluster") {
    save(
      output,
      file = paste0(
        "/gscratch/csde/jiang14/output/sensitivity/",
        threshold_names[thres_id],"/mhpvisgdtr_seed_",
        seed_id,
        "_dataset_" ,
        dat_id,
        "newinit_time30.RData"
      )
    )
  }
}
