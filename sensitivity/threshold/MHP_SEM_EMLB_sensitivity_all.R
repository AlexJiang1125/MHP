...
pars <- expand.grid(seed_id = 1:16, dat_id = 1:50, thres_id = 1:6)
...
for (dat_id in dat_id: dat_id) {
  ...
  #load(paste0("/gscratch/home/jiang14/mhp/data/sim001/data/data_",dat_id,".RData"))
  set.seed(seed_id)
  time_EMLB <- 0
  sgd_par1 <- 5
  sgd_par2 <- 0.51
  ...
  flag <- 0
  stepsize <- 0.02
  thresholds <- c(0.125, 0.5, 0.75, 1, 0.25, 0)
  threshold_names <- c("t05", "t01", "t005", "t001", "t0", "t00")
  ...
  if (type == "cluster") {
    if (mode == "long") {
      ...
      save(
        output,
        file = paste0(
          "/gscratch/csde/jiang14/output_short/sensitivity",
          files[split_id],
          "/mhpvisemtr_seed_",
          seed_id,
          "_dataset_" ,
