...
compensator_mcmc <- function(results,dim,out, method) {
  ...
  out_test <- out#[out$timestamp < 700,]
  #out_test <- out_test[out_test$timestamp > 300,]
  ...
  for (iter in 1:iters) {
    ...
    z_out_list[[iter]] <- tempfun(mu, alpha, beta)
  }
  return(z_out_list)
}
...
K <- 11
pars <- expand.grid(b = 1:50, dim = 1:K)
b <- pars$b[taskid]
...
plist <- list()
load("mhpvisgdtr_seed_10_dataset_1newinit_time30.RData")
res <- compensator_vi(output = output, dim = mydim, out = out)
fileoutname <- paste0("sgvitr_dim_", mydim, "_", taskid, ".RData")
save(res, file = file.path("output/sgvitr", fileoutname))
