...
compensator_ld <- function(results,dim,out,method,iter_id) {
  ...
  id_start <- (iter_id-1)*90 + 3001
  id_end <-iter_id*90 + 3000
...
K <- 11
pars <- expand.grid(b = 1:49, dim = 1:K)
b <- pars$b[taskid]
...
NAME_PREFIX <- c("mhpvi")
NAME_SUFFICES <- c("_time30.RData", "_time30.RData", "newinit_time30.RData", "newinit_time30.RData", "_newinit_time_s001_120.RData")
...
plist <- list()
load("mhpvisgld_seed_12_dataset_1_newinit_time_s001_120.RData")
res <- compensator_ld(results = output_app, dim = mydim, out = out, iter_id = b)
fileoutname <- paste0("sgld_dim_", mydim, "_", b, ".RData")
save(res, file = file.path("output/sgld", fileoutname))